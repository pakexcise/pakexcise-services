...
export async function generateMetadata({
  params}: SupportGuestLeadDetailPageProps): Promise<Metadata> {
...
export default async function SupportGuestLeadDetailPage({
  params}: SupportGuestLeadDetailPageProps) {
  const { id } = await params;
  const locale = "en";
  const t = createT(copy.admin);
  ...
  const serviceName = lead.serviceNameEn ?? "";
  const regionName = lead.regionNameEn ?? "";

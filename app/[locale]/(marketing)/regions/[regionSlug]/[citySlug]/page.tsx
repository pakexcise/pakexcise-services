  ...
  return await resolveMetadataFromSeo({
    ...
    fallbacks: {
      title: {
        ...
        ur: `${city.nameUr} ایکسائز خدمات | PakExcise.com`,
      },
      description: {
        en: city.descriptionEn ?? city.nameEn,
        ur: city.descriptionUr ?? city.nameUr,
      },
      h1: { en: city.nameEn, ur: city.nameUr },
    },
  });
}
...
export default async function CityDetailPage({ params }: CityPageProps) {
  ...
  if (canonicalRegionSlug !== regionSlug) {
    redirect({
      href: `/regions/${canonicalRegionSlug}/${citySlug}`,
      locale,
    });
  }
  ...
  const [services, faqs, business] = await Promise.all([
    serviceRepository.listPublicByRegionId(region.id),
    faqRepository.listPublic(),
    getBusinessSettings(),
  ]);

  const cityName = pickLocalized(locale, { en: city.nameEn, ur: city.nameUr });
  const regionName = pickLocalized(locale, {
    en: region.nameEn,
    ur: region.nameUr,
  });
  const description = pickLocalized(locale, {
    en: city.descriptionEn,
    ur: city.descriptionUr,
  });

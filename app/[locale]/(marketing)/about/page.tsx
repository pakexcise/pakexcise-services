  ...
  return await resolveMetadataFromSeo({
    ...
    fallbacks: {
      title: {
        ...
        ur: seo?.metaTitleUr ?? content?.titleUr ?? ABOUT_PAGE_SEO.metaTitleUr,
      },
      description: {
        en:
        ...
        ur:
          seo?.metaDescriptionUr ??
          content?.excerptUr ??
          ABOUT_PAGE_SEO.metaDescriptionUr,
      },
      h1: {
        en: seo?.h1En ?? content?.titleEn ?? ABOUT_PAGE_SEO.h1En,
        ur: seo?.h1Ur ?? content?.titleUr ?? ABOUT_PAGE_SEO.h1Ur,
      },
    },
  });
}
...
export default async function AboutPage() {
  ...
  const [seo, content, socialLinks, categoryGroups, business] = await Promise.all([
    seoMetaRepository.findByPageKey("about"),
    getPageContent("about"),
    getActiveSocialLinks(),
  ...
  const description = pickLocalized(locale, {
    en: content.excerptEn ?? ABOUT_PAGE_SEO.excerptEn,
    ur: content.excerptUr ?? ABOUT_PAGE_SEO.excerptUr,
  });
  const whoWeAreParagraphs = splitParagraphs(
    pickLocalized(locale, {
      en: content.contentEn,
      ur: content.contentUr,
    }),
  );
  const breadcrumbLabel = pickLocalized(locale, {
    en: "About",
    ur: "ہمارے بارے میں",
  });

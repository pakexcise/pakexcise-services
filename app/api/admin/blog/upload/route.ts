...
import { saveBlogPublicImage } from "@/features/blog/lib/upload-blog-image";
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    await enforceRateLimit(serverActionRateLimit, `blog-image-upload:${user.id}`);
  } catch {
  ...
  const result = await saveBlogPublicImage(
    buffer,
    fileEntry.name,
